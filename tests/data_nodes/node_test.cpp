...
template <typename T>
void comp_std_list_vs_llist(std::list<T> &std_list,Linked_list<T> &list)
{
...
void find_remove_test(size_t remove_postition)
{
    ...
    //Delete from he tail in the std list
    t_list.remove(g_data_array.at(remove_postition));
    //Look for tail it should not be there
...
TEST(Linked_list, addition_head_tail_checks)
{
...
TEST(Linked_list, copy_constructor_copy_assigment_test)
{
    //Linked list used for testing
    Linked_list<array_t> l_list {};
    ...
    Linked_list<array_t> l_list_copy{l_list};
    Linked_list<array_t> l_list_copy_assigned = l_list;
    ...
    Linked_list<array_t> secod_array_test {};
...
TEST(Linked_list, find_delete_head_mid_tail)
{
    //Remove form head
    find_remove_test(0);
    //Remove form middle
...
TEST(Linked_list, find_delete_empty)
{
    //Linked list used for testing
    Linked_list<array_t> l_list {};
    //Look for head
    EXPECT_EQ(l_list.is_in_list(g_data_array.at(0)),false);
